...
void aver_varian(const double tab[], size_t n, double *arith_average, double *variance) {
    ...
    average=average/n;
    *arith_average=average;
    for(int i=0; i<n; i++){
        var=var+(tab[i]-*arith_average)*(tab[i]-*arith_average);
    }
    var=var/n;
    *variance=var;
}
...
void two_dice_probab_distrib(double distrib[], int throws_num) {
    int a;
    int b;
    double c = 1.0/throws_num;
    for(int i=0; i<throws_num; i++){
        a = rand()%6 + 1;
        b = rand()%6 + 1;
        distrib[a+b-2]=distrib[a+b-2]+c;
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) {
    ...
    for(int j=0; j<n; j++){
        random_door = rand()%3;
        random_pick = rand()%3;
        while(random_hall==random_door || random_hall==random_pick){
            random_hall=rand()%3;
        }
        if(random_pick!=random_door){
            *p_switch_wins++;
        }
        if(random_pick==random_door){
            continue;
